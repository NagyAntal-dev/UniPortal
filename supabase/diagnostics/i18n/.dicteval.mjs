...
Object.entries({
  ...
  'Oktatónként és elvárásonként ismétlődő kérdés: minden oktatónál egyszer, és — akik a félév elején oktatói elvárást adtak meg — elvárásonként is egyszer. Az elvárásonkénti válaszok összevontan kerülnek be.': 'Repeated per teacher and per expectation: once for every teacher and — for respondents who stated expectations towards the teacher at the start of the semester — once per expectation as well. The per-expectation answers are submitted merged.',
  'Oktatónként és elvárásonként': 'Per teacher and per expectation',
}).forEach(([k, v]) => { if (!(k in HU_EN)) HU_EN[k] = v; });
...
export {HU_EN, HU_EN_PHRASES, STATUS_I18N};